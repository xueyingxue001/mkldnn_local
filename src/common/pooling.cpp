...
status_t pooling_desc_init(pooling_desc_t *pool_desc,
    ...
    bool args_ok = true
        ...
        && one_of(alg_kind, pooling_max, pooling_adaptiveAvg,
    ...
    bool effective_ndims = true
        && utils::one_of(src_desc->ndims, 4, 5)
        && utils::one_of(dst_desc->ndims, 4, 5);
    if (!effective_ndims) return invalid_arguments;
	dims_t tmp_strides = {0, 0}, tmp_kernel = {0, 0};
	if (alg_kind == pooling_adaptiveAvg) {
		int h_id = 2, ksh_id = 0;
		if (src_desc->ndims == 5) {
			h_id++;
			ksh_id++;
		...
		int w_id = h_id + 1;
		int ksw_id = ksh_id + 1;
		int ih = src_desc->dims[h_id];
		int iw = src_desc->dims[w_id];
		int oh = dst_desc->dims[h_id];
		int ow = dst_desc->dims[w_id];
		tmp_strides[ksh_id] = int(ih / oh);
		tmp_strides[ksw_id] = int(iw / ow);
		tmp_kernel[ksh_id] = ih - (oh - 1) * tmp_strides[ksh_id];
		tmp_kernel[ksw_id] = iw - (ow - 1) * tmp_strides[ksw_id];
	}
	...
	if (alg_kind == pooling_adaptiveAvg) {
		pd.alg_kind = pooling_avg_exclude_padding;
	} else {
	...
	if (alg_kind == pooling_adaptiveAvg) {
		utils::array_copy(pd.strides, tmp_strides, sp_dims);
		utils::array_copy(pd.kernel, tmp_kernel, sp_dims);
	} else {
    ...
    if (one_of(alg_kind, pooling_max, pooling_adaptiveAvg, pooling_avg_include_padding,
                pooling_avg_exclude_padding)) {
	...
	if (alg_kind == pooling_adaptiveAvg) {
		for (int i = 2; i < src_desc->ndims; ++i)
			consistency = consistency && (
					(src_desc->dims[i] - tmp_kernel[i - 2] + padding_l[i - 2]
					 + padding_r[i - 2]) / tmp_strides[i - 2] + 1
					== dst_desc->dims[i]);
	} else {
		for (int i = 2; i < src_desc->ndims; ++i)
			consistency = consistency && (
