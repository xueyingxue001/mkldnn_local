...
#else
        const float top_left = src[src_d.off(_b, _c, ys[_oh].lower, xs[_ow].lower)];
        const float top_right = src[src_d.off(_b, _c, ys[_oh].lower, xs[_ow].upper)];
        const float bottom_left = src[src_d.off(_b, _c, ys[_oh].upper, xs[_ow].lower)];
        const float bottom_right = src[src_d.off(_b, _c, ys[_oh].upper, xs[_ow].upper)];

        float top = top_left + (top_right - top_left) * xs[_ow].lerp;
        float bottom = bottom_left + (bottom_right - bottom_left) * xs[_ow].lerp;

        dst[dst_d.off(_b, _c, _oh, _ow)] = top + (bottom - top) * ys[_oh].lerp;
#endif
    };
